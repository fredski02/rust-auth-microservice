...
impl Users for UsersImpl {
    fn create_user(&mut self, username: String, password: String) -> Result<(), String> {
        ...
        let already_exists = self.username_to_user.get(&username).is_some();
        if already_exists {
        ...
        self.uuid_to_user.insert(user_uuid.clone(), user);
    ...
    fn get_user_uuid(&self, username: String, password: String) -> Option<String> {
        ...
        let result = Pbkdf2.verify_password(password.as_bytes(), &parsed_hash).ok();
